...
</VirtualHost>' | sudo tee /etc/apache2/sites-available/laravelnew.conf
sudo a2ensite laravelnew.conf
...
sudo mysql -uroot -e "CREATE USER 'Daniel'@'localhost' IDENTIFIED BY 'Tricia';"
sudo mysql -uroot -e "GRANT ALL PRIVILEGES ON Altschool.* TO 'Daniel'@'localhost';"
cd /var/www/laravel
...
sudo sed -i '26 s/=root/=Daniel/' /var/www/laravel/.env
sudo sed -i '27 s/=/=Tricia/' /var/www/laravel/.env
